...
makeGOdata <- function(dataFiles="dataFiles",ontology="MF"){
  ...
  for (f in dataFiles){
    ...
    atac.cat.label = atac_cat_lev[which(!(atac_cat_lev=="cat2" | atac_cat_lev=="background"))]
    
    #atac.cat.label = levels(atac_cat[which(!=background)])
    ...
    res<- print(GenTable( GOdata, topGO = resultTopgo, orderBy = "topGO", ranksOf = "fisher", topNodes = 50),sep = "\t", quotes = FALSE, 
                row.names = FALSE)
    res$atac.seq.category <- atac.cat.label
    ...
    # output a table with all GO analyses in one with an added column for atac-seq category
    write.table(combined.GO.output, paste("Routput/GO/combinedGOoutput.", ontology, ".txt", sep = ""), quote = FALSE, sep = "\t", row.names = FALSE)
  }
}
